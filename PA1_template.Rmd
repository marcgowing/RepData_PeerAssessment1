...
```{r librarychunk, echo=TRUE, results="hide", warning=FALSE, message=FALSE}
# load libraries
library(dplyr)
library(lattice)
```
...
Interval **`r maxInterval`** has the largest 5-minute mean across all days.
...
There are `r totalNumRowsWithNAs` rows with NAs in the dataset.
...
```{r histogramGapsFilled, echo=TRUE}
...
```{r weekendOrWeekdayPlot, echo=TRUE}
